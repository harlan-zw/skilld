    ...
    packages: {
      vue: {
        ...
        rules: [
          ...
          "Use ```vue code fences for SFC examples containing `<script>` or `<template>` tags, ```ts for plain TypeScript",
        ],
      },
      "@vue/compiler-core": {},
      "@vue/compiler-dom": {},
...
export function getDocOverride(packageName: string): DocOverride | undefined {
  ...
  if (!entry?.docsRepo && !entry?.docsPath) return undefined;
...
export function getBlogPreset(packageName: string): BlogPreset | undefined {
  ...
  if (!repoKey) return undefined;
  const entry = REPO_REGISTRY[repoKey];
  if (!entry?.blogReleases) return undefined;
