...
async function findGitTag(
  ...
  for (const tag of candidates) {
    ...
    if (files.length > 0) return { ref: tag, files };
  }
  ...
  if (packageName) {
    ...
    if (latestTag) {
      ...
      if (files.length > 0) return { ref: latestTag, files };
    }
  }
  ...
  for (const branch of branches) {
    ...
    if (files.length > 0) return { ref: branch, files, fallback: true };
  }
 ...
 */
async function fetchUnghReleases(
  owner: string,
  repo: string,
): Promise<Array<{ tag: string; publishedAt?: string }>> {
  if (!isKnownPrivateRepo(owner, repo)) {
...
const NOISE_PATTERNS = [/^\.changeset\//, /CHANGELOG\.md$/i, /CONTRIBUTING\.md$/i, /^\.github\//];
...
function discoverDocFiles(allFiles: string[], packageName?: string): DiscoveredDocs | null {
  ...
  if (packageName?.includes("/")) {
    ...
    if (subPkgFiles.length >= 3) return { files: subPkgFiles, prefix: subPkgPrefix };
  }
 ...
 */
export async function fetchGitDocs(
  owner: string,
  repo: string,
  version: string,
  packageName?: string,
  repoUrl?: string,
): Promise<GitDocsResult | null> {
...
export function validateGitDocsWithLlms(
  ...
  if (llmsLinks.length === 0) return { isValid: true, matchRatio: 1 };
...
export async function searchGitHubRepo(packageName: string): Promise<string | null> {
  ...
  for (const candidate of [packageName.replace(/^@/, "").replace("/", "/"), shortName]) {
    ...
    if (!candidate.includes("/")) {
      ...
      const unghRes = await $fetch
        .raw(`https://ungh.cc/repos/${shortName}/${shortName}`)
        .catch(() => null);
      if (unghRes?.ok) return `https://github.com/${shortName}/${shortName}`;
      continue;
    }
    const unghRes = await $fetch.raw(`https://ungh.cc/repos/${candidate}`).catch(() => null);
    if (unghRes?.ok) return `https://github.com/${candidate}`;
  }
  ...
  if (isGhAvailable()) {
    try {
      const { stdout: json } = spawnSync(
        "gh",
        ["search", "repos", searchTerm, "--json", "fullName", "--limit", "5"],
        {
          encoding: "utf-8",
          timeout: 15_000,
        },
...
export async function fetchGitHubRepoMeta(
  ...
  const data =
    (await ghApi<{ homepage?: string }>(`repos/${owner}/${repo}`)) ??
    (await $fetch<{ homepage?: string }>(`https://api.github.com/repos/${owner}/${repo}`).catch(
      () => null,
    ));
...
export async function fetchReadme(
  ...
  // Try ungh first (skip for known private repos)
  if (!isKnownPrivateRepo(owner, repo)) {
...
export async function fetchReadmeContent(url: string): Promise<string | null> {
  ...
  // For raw.githubusercontent.com URLs, use auth-aware fetch (private repo support)
...
export async function resolveGitHubRepo(
  ...
  const meta =
    (await ghApi<{ homepage?: string; description?: string }>(`repos/${owner}/${repo}`)) ??
    (await $fetch<{ homepage?: string; description?: string }>(
