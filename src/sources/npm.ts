...
export async function searchNpmPackages(
  ...
  }>(`https://registry.npmjs.org/-/v1/search?text=${encodeURIComponent(query)}&size=${size}`).catch(
    () => null,
  );
...
export async function fetchNpmPackage(packageName: string): Promise<NpmPackageInfo | null> {
  ...
  const data = await $fetch<NpmPackageInfo>(`https://unpkg.com/${packageName}/package.json`).catch(
    () => null,
  );
  if (data) return data;

  // Fallback to npm registry
  return $fetch<NpmPackageInfo>(`https://registry.npmjs.org/${packageName}/latest`).catch(
    () => null,
  );
}
 ...
 */
export async function fetchNpmRegistryMeta(
  packageName: string,
  version: string,
...
async function resolveGitHub(
  ...
  if (targetVersion) {
    ...
    const gitDocs = await fetchGitDocs(
      gh.owner,
      gh.repo,
      targetVersion,
      pkg.name,
 ...
 */
export async function resolvePackageDocs(
  packageName: string,
 ...
 */
export async function resolvePackageDocsWithAttempts(
  packageName: string,
  options: ResolveOptions = {},
  ...
  if (
    ...
    const readmeFile =
      existsSync(pkgDir) && readdirSync(pkgDir).find((f) => /^readme\.md$/i.test(f));
    if (readmeFile) {
...
export function parseVersionSpecifier(
  ...
  if (installed) return { name, version: installed };

  // Fallback: strip semver prefix if it looks like one
  if (/^[\^~>=<\d]/.test(version)) return { name, version: version.replace(/^[\^~>=<]+/, "") };
...
export async function fetchPkgDist(name: string, version: string): Promise<string | null> {
  ...
  const { status } = spawnSync("tar", ["xzf", tmpTarball, "--strip-components=1", "-C", pkgDir], {
    stdio: "ignore",
  });
  if (status !== 0) {
