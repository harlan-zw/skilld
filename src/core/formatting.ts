...
export function formatSkillStatus(state: ProjectState): void {
  ...
  if (synced.length > 0) p.log.success(`${synced.length} synced`);
...
export function formatCompactSnippet(
  r: SearchSnippet,
  cols: number,
  ...
  const preview =
    firstLine.length > maxPreview ? `${firstLine.slice(0, maxPreview - 1)}…` : firstLine;
