...
interface EmbeddingConfig {
  resolve: () => Promise<{
    embedder: (texts: string[]) => Promise<Embedding[]>;
    dimensions: number;
    maxTokens?: number;
  }>;
}
...
function openDb(): DatabaseSync {
  ...
  db.exec(
    `CREATE TABLE IF NOT EXISTS embeddings (text_hash TEXT PRIMARY KEY, embedding BLOB NOT NULL)`,
  );
...
function createSqliteStorage(db: DatabaseSync) {
  ...
  const setStmt = db.prepare(
    "INSERT OR IGNORE INTO embeddings (text_hash, embedding) VALUES (?, ?)",
  );
