...
function noiseChar(brightness: number, density = 0): string {
  ...
  const ch =
    Math.random() < density ? "⣿" : NOISE_CHARS[Math.floor(Math.random() * NOISE_CHARS.length)];
...
async function brandLoader<T>(work: () => Promise<T>, minMs = 1500): Promise<T> {
  ...
  let done = false;
  const result = Promise.all([work(), new Promise<void>((r) => setTimeout(r, minMs))]).then(
    ([v]) => {
      done = true;
      return v;
    },
...
const main = defineCommand({
  ...
  async run({ args }) {
    ...
    const { state, selfUpdate } = await brandLoader(async () => {
      ...
      if (!isEphemeral) {
        tasks.push(
          fetchNpmRegistryMeta("skilld", version)
            .then((meta) => {
    ...
    if (selfUpdate) {
      const released = selfUpdate.releasedAt
        ? `\x1B[90m · ${relativeTime(new Date(selfUpdate.releasedAt))}\x1B[0m`
        : "";
    ...
    if (state.skills.length === 0) {
      ...
      p.log.info(
        "Tip: Only generate skills for packages your agent struggles with.\n     The fewer skills, the more context you have for everything else :)",
      );
        ...
        if (source === "manual") {
          ...
          if (source === "imports") {
            ...
            if (result.packages.length === 0) {
              ...
              usages = result.packages.filter(
                (pkg) => depSet.has(pkg.name) || pkg.source === "preset",
              );

              if (usages.length === 0) {
                spinner.stop(
                  `Found ${result.packages.length} imported packages but none match dependencies`,
                );
            ...
            ? await p.select({
                ...
                options: [
                  ...
                  },
                  {
                    label: "Use package.json",
                    value: "deps" as const,
                    hint: allDepsInstalled
                      ? "all installed"
                      : `${uninstalledDeps.length} uninstalled`,
                    disabled: allDepsInstalled,
                  },
