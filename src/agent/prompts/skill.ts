...
export function generateSkillMd(opts: SkillOptions): string {
  ...
  const search =
    !opts.eject && opts.features?.search !== false
      ? generateSearchBlock(opts.name, opts.hasIssues, opts.hasReleases)
      : "";
...
function generateFrontmatter({
  ...
  if (packages && packages.length > 1) {
    ...
    if (repoUrl) {
      for (const kw of expandRepoName(repoUrl)) allKeywords.add(kw);
    }
