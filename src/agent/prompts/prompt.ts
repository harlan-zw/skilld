...
export function extractMarkedSections(
  md: string,
): Map<SkillSection, { start: number; end: number }> {
  ...
  for (const section of SECTION_MERGE_ORDER) {
    ...
    if (start !== -1 && end !== -1) sections.set(section, { start, end: end + close.length });
  }
  return sections;
}
...
function generateImportantBlock({
  packageName,
  hasIssues,
  hasDiscussions,
  hasReleases,
  ...
  if (typesFile) {
    rows.push([
      "Types",
      `\`${skillDir}/.skilld/pkg/${typesFile}\` — **read this file directly** to verify exports`,
    ]);
  }
  if (hasIssues) {
...
function getSectionDef(
  ...
  switch (section) {
    case "api-changes":
      return apiChangesSection(ctx);
    case "best-practices":
      return bestPracticesSection(ctx);
    case "custom":
      return customPrompt ? customSection(customPrompt, ctx.enabledSectionCount) : null;
  }
}
 ...
 */
export function getSectionValidator(
  section: SkillSection,
): ((content: string) => SectionValidationWarning[]) | null {
...
export function buildSectionPrompt(
  opts: BuildSkillPromptOptions & { section: SkillSection },
): string {
  const {
    packageName,
    hasIssues,
 ...
 */
export function buildAllSectionPrompts(
  opts: BuildSkillPromptOptions & { sections: SkillSection[] },
