...
export function classifyCachedDoc(path: string): { type: string; number?: number } {
  ...
  if (issueMatch) return { type: "issue", number: Number(issueMatch[1]) };
  const discussionMatch = path.match(/^discussions\/discussion-(\d+)\.md$/);
  if (discussionMatch) return { type: "discussion", number: Number(discussionMatch[1]) };
...
export function forceClearCache(
  ...
  if (existsSync(forcedDbPath)) rmSync(forcedDbPath, { recursive: true, force: true });
...
export function linkAllReferences(
  skillDir: string,
  packageName: string,
  cwd: string,
  version: string,
  ...
  try {
    ...
    if (f.issues) {
      if (repoInfo) linkRepoCachedDir(skillDir, repoInfo.owner, repoInfo.repo, "issues");
      else linkCachedDir(skillDir, packageName, version, "issues");
    }
    if (f.discussions) {
      if (repoInfo) linkRepoCachedDir(skillDir, repoInfo.owner, repoInfo.repo, "discussions");
      else linkCachedDir(skillDir, packageName, version, "discussions");
    }
    if (f.releases) {
      if (repoInfo) linkRepoCachedDir(skillDir, repoInfo.owner, repoInfo.repo, "releases");
    ...
    if (extraPackages) {
      for (const pkg of extraPackages) {
        if (pkg.name !== packageName) linkPkgNamed(skillDir, pkg.name, cwd, pkg.version);
      }
    }
  } catch {
...
export function detectDocsType(
  packageName: string,
  version: string,
  repoUrl?: string,
  llmsUrl?: string,
  ...
  if (
    existsSync(join(cacheDir, "docs", "index.md")) ||
    existsSync(join(cacheDir, "docs", "guide"))
  ) {
    return {
      docsType: "docs",
...
export async function resolveLocalDep(
  packageName: string,
  cwd: string,
...
export async function fetchAndCacheResources(opts: {
  ...
  if (!useCache) {
      ...
      if (gh) {
        ...
        if (gitDocs && gitDocs.files.length > 0) {
          ...
          for (let i = 0; i < gitDocs.files.length; i += BATCH_SIZE) {
            ...
            onProgress(
              `Downloading docs ${Math.min(i + BATCH_SIZE, gitDocs.files.length)}/${gitDocs.files.length} from ${gitDocs.ref}`,
            );
              ...
              if (resolved.llmsUrl) {
                ...
                if (llmsContent) {
                  ...
                  if (llmsContent.links.length > 0) {
                    ...
                    const docs = await downloadLlmsDocs(
                      llmsContent,
                      baseUrl,
                      (url, done, total) => {
    ...
    if (resolved.docsUrl && !cachedDocs.some((d) => d.path.startsWith("docs/"))) {
      ...
      const crawledDocs = await fetchCrawledDocs(crawlPattern, onProgress, crawlMaxPages).catch(
        (err) => {
    ...
    if (
      docsType !== "readme" &&
      cachedDocs.filter((d) => d.path.startsWith("docs/") && d.path.endsWith(".md")).length > 1
    ) {
    ...
    if (
      docsType !== "readme" &&
      !existsSync(join(getCacheDir(packageName, version), "docs", "_INDEX.md"))
    ) {
  ...
  const discussionsDir = repoCacheDir
    ? join(repoCacheDir, "discussions")
    : join(cacheDir, "discussions");
  ...
  if (features.releases && gh && isGhAvailable() && !existsSync(releasesPath)) {
    ...
    const releaseDocs = await fetchReleaseNotes(
      gh.owner,
      gh.repo,
      version,
      resolved.gitRef,
    ...
    await createIndex(allDocs, {
      ...
      onProgress: ({ phase, current, total }) => {
        if (phase === "storing") {
          ...
          const type =
            d?.metadata?.type === "source" || d?.metadata?.type === "types"
              ? "code"
              : d?.metadata?.type || "doc";
 ...
 */
export function ejectReferences(
  skillDir: string,
  packageName: string,
  cwd: string,
  version: string,
...
export async function ensureAgentInstructions(
  ...
  if (existsSync(filePath)) {
    ...
    if (content.includes(SKILLD_MARKER_START)) return;
  }
...
export async function selectSkillSections(
  message = "Generate SKILL.md with LLM",
): Promise<{ sections: SkillSection[]; customPrompt?: CustomPrompt; cancelled: boolean }> {
  ...
  const selected = await p.multiselect({
    ...
    options: [
      {
        label: "API changes",
        value: "api-changes" as SkillSection,
        hint: "new/deprecated APIs from version history",
      },
      {
        label: "Best practices",
        value: "best-practices" as SkillSection,
  ...
  if (p.isCancel(selected)) return { sections: [], cancelled: true };

  const sections = selected as SkillSection[];
  if (sections.length === 0) return { sections: [], cancelled: false };
  ...
  if (sections.length > 1) {
    ...
    for (const s of sections) {
      switch (s) {
        case "api-changes":
          budgetLines.push(
            `  API changes     ${maxItems(6, 12, n)}–${maxItems(6, Math.round(12 * 1.6), n)} items (adapts to release churn)`,
          );
          break;
        case "best-practices":
          budgetLines.push(
            `  Best practices  ${maxItems(4, 10, n)}–${maxItems(4, Math.round(10 * 1.3), n)} items`,
          );
          break;
        case "custom":
 ...
 */
export async function selectLlmConfig(
  presetModel?: OptimizeModel,
  message?: string,
...
export async function enhanceSkillWithLLM(opts: EnhanceOptions): Promise<void> {
  ...
  const { optimized, wasOptimized, usage, cost, warnings, error, debugLogsDir } =
    await optimizeDocs({
      packageName,
      skillDir,
      model,
