...
function discoverSkillDirsWithOutputs(): string[] {
  ...
  for (const skill of iterateSkills({})) {
    if (readdirSync(skill.dir).some((f) => OUTPUT_FILE_SET.has(f))) dirs.push(skill.dir);
  }
  return dirs;
}
...
export async function assembleCommand(dir: string | undefined): Promise<void> {
  ...
  if (dir) {
    ...
    if (existsSync(join(cwd, "SKILL.md")) && readdirSync(cwd).some((f) => OUTPUT_FILE_SET.has(f))) {
      dirs = [cwd];
    } else {
  ...
  for (const targetDir of dirs) assembleDir(targetDir, cwd);
}
...
function assembleDir(targetDir: string, cwd: string): void {
  ...
  for (const [section, outputFile] of Object.entries(SECTION_OUTPUT_FILES) as Array<
    [SkillSection, string]
  >) {
    ...
    if (validator) {
      for (const w of validator(cleaned)) warnings.push(`${section}: ${w.warning}`);
    }
  ...
  if (sections.length === 0) {
    p.log.warn(
      `No section output files in ${relative(cwd, targetDir)}. Expected: ${Object.values(SECTION_OUTPUT_FILES).join(", ")}`,
    );
    return;
  }
  ...
  for (const section of SECTION_MERGE_ORDER) {
    ...
    if (result) wrappedSections.push({ section, wrapped: wrapSection(section, result.content) });
  }
  ...
  if (existingMarkers.size > 0) {
    ...
    const replacements = wrappedSections
      ...
      .sort(
        (a, b) => existingMarkers.get(b.section)!.start - existingMarkers.get(a.section)!.start,
      );
