...
async function countEmbeddings(packageName: string, version?: string): Promise<number | null> {
  ...
  try {
    ...
    try {
      const row = db.prepare("SELECT count(*) as cnt FROM vector_metadata").get() as
        | { cnt: number }
        | undefined;
      return row?.cnt ?? null;
    } finally {
      ...
      for (const entry of readdirSync(dir, { withFileTypes: true })) {
        if (entry.isDirectory() || entry.isSymbolicLink()) {
          try {
            ...
            if (stat.isDirectory()) walk(join(dir, entry.name), depth + 1);
          } catch {
            continue;
          }
...
function buildConfigLines(): string[] {
  ...
  lines.push(
    `Config    ${dim(join(CACHE_DIR, "config.yaml"))}${hasConfig() ? "" : dim(" (not created)")}`,
  );
  lines.push(`Cache     ${dim(CACHE_DIR)}`);

  const withCli = Object.entries(agents).filter(([_, a]) => a.cli) as [
    AgentType,
...
export async function statusCommand(opts: StatusOptions = {}): Promise<void> {
  ...
  const buildPackageLines = async (pkgs: Map<string, TrackedPackage>): Promise<string[]> => {
    ...
    for (const [, pkg] of pkgs) {
      ...
      const docs =
        countDocs(pkgName, info.version) ||
        countRefDocs(
          join(
            pkg.scope === "global"
