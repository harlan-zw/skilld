...
export async function removeCommand(state: ProjectState, opts: RemoveOptions): Promise<void> {
  ...
  if (!isInteractive() && !opts.packages) {
    console.error(
      "Error: `skilld remove` requires package names in non-interactive mode.\n  Usage: skilld remove <package...>",
    );
    process.exit(1);
  }
  ...
  for (const skill of skills) {
    ...
    if (existsSync(skill.dir)) {
      ...
      if (shared && skill.scope === "local") unlinkSkillFromAgents(skill.name, cwd, opts.agent);
...
async function pickSkillsToRemove(
  skills: SkillEntry[],
  scope: "local" | "global",
