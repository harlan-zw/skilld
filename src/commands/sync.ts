...
async function interactivePicker(state: ProjectState): Promise<string[] | null> {
  ...
  if (error || detected.length === 0) {
    ...
    return pickFromList(
      Array.from(declaredMap.entries(), ([name, version]) => ({
        name,
...
async function pickFromList(
  ...
  const options = packages.map((pkg) => ({
    ...
    value: pkg.name,
    hint:
      [maskPatch(pkg.version), pkg.count > 0 ? `${pkg.count} imports` : null]
        .filter(Boolean)
        .join(" · ") || undefined,
  }));
...
async function syncSinglePackage(packageSpec: string, config: SyncConfig): Promise<void> {
  ...
  const resolveResult = await resolvePackageDocsWithAttempts(
    requestedTag ? packageSpec : packageName,
    {
  ...
  if (shippedResult) {
    ...
    for (const shipped of shippedResult.shipped) {
      ...
      p.log.success(
        `Using published SKILL.md: ${shipped.skillName} → ${relative(cwd, shipped.skillDir)}`,
      );
    }
  ...
  spin.stop(
    `Resolved ${packageName}@${useCache ? versionKey : version}${config.force ? " (force)" : useCache ? " (cached)" : ""}`,
  );
  ...
  if (!localVersion && !requestedTag && !isPrerelease(version)) {
    ...
    if (
      nextTag &&
      (!resolved.releasedAt || !nextTag.releasedAt || nextTag.releasedAt > resolved.releasedAt)
    ) {
      p.log.warn(
        `\x1B[33mNo local dependency found — using latest stable (${version}). Prerelease ${nextTag.version} available: skilld add ${packageName}@beta\x1B[0m`,
      );
    }
  }
  ...
  const isMerge =
    existingLock && existingLock.packageName && existingLock.packageName !== packageName;
  ...
  if (isMerge) {
    ...
    // Merge into lockfile
    const repoSlug = resolved.repoUrl?.match(
      /github\.com\/([^/]+\/[^/]+?)(?:\.git)?(?:[/#]|$)/,
    )?.[1];
    ...
    const skillMd = generateSkillMd({
      ...
      hasDiscussions:
        mergeFeatures.discussions && existsSync(join(skillDir, ".skilld", "discussions")),
      hasReleases: mergeFeatures.releases && existsSync(join(skillDir, ".skilld", "releases")),
      docsType: (existingLock.source?.includes("llms.txt") ? "llms.txt" : "docs") as
        | "llms.txt"
        | "readme"
        | "docs",
  ...
  p.log.success(
    config.mode === "update"
      ? `Updated skill: ${relative(cwd, skillDir)}`
      : `Created base skill: ${relative(cwd, skillDir)}`,
  );
  ...
  if (isEject) {
    ...
    if (existsSync(skilldDir) && !config.debug) rmSync(skilldDir, { recursive: true, force: true });
    ejectReferences(
      skillDir,
      packageName,
      cwd,
  ...
  if (!isEject) {
    ...
    await ensureGitignore(
      shared ? SHARED_SKILLS_DIR : agents[config.agent].skillsDir,
      cwd,
      config.global,
    );
  ...
  p.outro(
    config.mode === "update"
      ? `Updated ${packageName}${ejectMsg}`
      : `Synced ${packageName} to ${relative(cwd, skillDir)}${ejectMsg}`,
  );
}
...
export const addCommandDef = defineCommand({
  ...
  async run({ args }) {
    ...
    const rawInputs = [
      ...new Set(
        [args.package, ...((args as any)._ || [])].map((s: string) => s.trim()).filter(Boolean),
      ),
    ];
...
export const updateCommandDef = defineCommand({
  ...
  args: {
    package: {
      type: "positional",
      description:
        "Package(s) to update (space or comma-separated). Without args, syncs all outdated.",
      required: false,
    },
    background: {
      type: "boolean",
...
export async function exportPortablePrompts(
  packageSpec: string,
  opts: {
    out?: string;
  ...
  const baseDir = agent ? resolveBaseDir(cwd, agent, false) : join(cwd, ".claude", "skills"); // fallback when no agent detected
  ...
  if (existsSync(skillDir) && !opts.force) {
    const existing = Object.values(SECTION_OUTPUT_FILES).filter((f) =>
      existsSync(join(skillDir, f)),
    );
    if (existing.length > 0)
      p.log.warn(
        `Overwriting existing output files in ${relative(cwd, skillDir)}: ${existing.join(", ")}`,
      );
  }
  mkdirSync(skillDir, { recursive: true });
  ...
  const skilldDir = join(skillDir, ".skilld");
  if (existsSync(skilldDir)) rmSync(skilldDir, { recursive: true, force: true });
