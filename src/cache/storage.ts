 ...
 * Write docs to cache
 */
export function writeToCache(name: string, version: string, docs: CachedDoc[]): string {
 ...
 */
export function writeToRepoCache(owner: string, repo: string, docs: CachedDoc[]): string {
 ...
 */
export function linkRepoCachedDir(
  skillDir: string,
  owner: string,
  repo: string,
  subdir: string,
...
export function getPkgKeyFiles(name: string, cwd: string, version?: string): string[] {
  ...
  const entries = readdirSync(pkgPath).filter(
    (f) => /^readme\.md$/i.test(f) || /^changelog\.md$/i.test(f),
  );
  files.push(...entries);
...
export function getShippedSkills(name: string, cwd: string, version?: string): ShippedSkill[] {
  ...
  return readdirSync(skillsPath, { withFileTypes: true })
    .filter(
      (d) =>
        d.isDirectory() &&
        (existsSync(join(skillsPath, d.name, "SKILL.md")) ||
          existsSync(join(skillsPath, d.name, "_SKILL.md"))),
    )
...
export function writeSections(
  name: string,
  version: string,
  sections: Array<{ file: string; content: string }>,
): void {
  const cacheDir = getCacheDir(name, version);
