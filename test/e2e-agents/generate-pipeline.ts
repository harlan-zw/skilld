...
export async function runGenerate(
  ...
  const tmpBase = join(
    tmpdir(),
    "skilld-generate-test",
    `${sanitizeName(packageName)}-${generator}`,
  );
...
export function writeGenerateArtifact(result: GenerateResult): void {
  ...
  if (result.optimizedBody) writeFileSync(join(dir, "body.md"), result.optimizedBody);
}

export function writeInstallArtifact(result: GenerateResult, install: InstallResult): void {
  const safePkg = result.package.replace(/\//g, "__");
