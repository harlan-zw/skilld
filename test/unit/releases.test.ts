...
import {
  isChangelogRedirectPattern,
  isPrerelease,
  isStubRelease,
  selectReleases,
} from "../../src/sources/releases";
...
describe("isChangelogRedirectPattern", () => {
  ...
  it("detects Vite-style changelog redirects", () => {
    const releases = [
      ...
      makeRelease(
        "v7.2.7",
        "Please refer to [CHANGELOG.md](https://github.com/vitejs/vite/blob/v7.2.7/packages/vite/CHANGELOG.md) for details.",
      ),
    ];
    expect(isChangelogRedirectPattern(releases)).toBe(true);
  });
  ...
  it("rejects releases with long markdown even if they mention changelog", () => {
    ...
    const releases = [makeRelease("v1.0.0", longContent), makeRelease("v0.9.0", longContent)];
    expect(isChangelogRedirectPattern(releases)).toBe(false);
  });
...
describe("isStubRelease", () => {
  it("detects stub with CHANGELOG.md reference", () => {
    expect(isStubRelease(makeRelease("v4.5.0", "Please refer to CHANGELOG.md for details."))).toBe(
      true,
    );
  });
  ...
  it("rejects real release notes", () => {
    expect(
      isStubRelease(
        makeRelease(
          "v4.3.0",
          "## What's Changed\n\n### Features\n\n- New routing system\n\n**Full Changelog**: https://github.com/org/repo/compare/v4.2.0...v4.3.0",
        ),
      ),
    ).toBe(false);
  });
