...
describe("sanitizeMarkdown", () => {
  ...
  describe("dangerous tags", () => {
    it("strips <system> tags and content", () => {
      expect(sanitizeMarkdown("before <system>evil instructions</system> after")).toBe(
        "before  after",
      );
    });
  ...
  describe("uRL sanitization", () => {
    ...
    it("preserves relative links", () => {
      expect(sanitizeMarkdown("[guide](./.skilld/docs/guide.md)")).toBe(
        "[guide](./.skilld/docs/guide.md)",
      );
  ...
  describe("integration", () => {
    it("does not modify clean SKILL.md body", () => {
      ...
      if (!existsSync(path)) return; // skip if no skills installed
...
describe("repairMarkdown", () => {
  describe("unclosed fenced code blocks", () => {
    ...
    it("auto-closes fence when new opener appears inside (fence-in-fence)", () => {
      const input = ["```ts", "const x = 1", "", "```md", "---", "key: value", "---", "```"].join(
        "\n",
      );
      const result = repairMarkdown(input);
      // Should insert ``` before the ```md line
      expect(result).toBe(
        ["```ts", "const x = 1", "", "```", "```md", "---", "key: value", "---", "```"].join("\n"),
      );
    });

    it("handles multiple consecutive fence-in-fence gaps", () => {
      const input = ["```ts", "code1", "```js", "code2", "```py", "code3", "```"].join("\n");
      const result = repairMarkdown(input);
    ...
    it("auto-closes code block when emoji starts a new line", () => {
      const input = ["```ts", "const x = ref(0)", "", "✅ Use ref() for reactive state"].join("\n");
      const result = repairMarkdown(input);
    ...
    it("handles multiple emoji lines after code", () => {
      const input = ["```ts", "const foo = bar()", "✅ Thing one", "⚠️ Thing two", "```"].join(
        "\n",
      );
      const result = repairMarkdown(input);
      expect(result).toContain("```ts\nconst foo = bar()\n```");
  ...
  describe("code block cleanup", () => {
    ...
    it("removes duplicate consecutive code blocks", () => {
      const input = ["```ts", "const x = 1", "```", "", "```ts", "const x = 1", "```"].join("\n");
      const result = repairMarkdown(input);
    ...
    it("preserves non-duplicate code blocks", () => {
      const input = ["```ts", "const x = 1", "```", "", "```ts", "const y = 2", "```"].join("\n");
      const result = repairMarkdown(input);
