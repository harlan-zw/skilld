...
function createMockFetch() {
  async function $fetch(url: string, opts?: any): Promise<any> {
    ...
    if (!mockRes?.ok) throw new Error("fetch failed");
    if (opts?.responseType === "text") return mockRes.text();
    return mockRes.json();
  }
...
describe("sources/npm", () => {
  describe("readLocalDependencies", () => {
    ...
    it("throws if package.json not found", async () => {
      ...
      await expect(readLocalDependencies("/test")).rejects.toThrow("No package.json found");
    });
    ...
    it("resolves catalog: and workspace: via installed version", async () => {
      ...
      vi.mocked(readFileSync).mockImplementation((p: any) => {
        const s = String(p);
        if (s.includes("node_modules/@clack/prompts")) return JSON.stringify({ version: "1.0.0" });
        if (s.includes("node_modules/citty")) return JSON.stringify({ version: "0.2.1" });
        if (s.includes("node_modules/bumpp")) return JSON.stringify({ version: "10.4.1" });
        return JSON.stringify({
          dependencies: {
  ...
  describe("resolveInstalledVersion", () => {
    ...
    it("resolves version from installed package.json", async () => {
      ...
      expect(vi.mocked(resolvePathSync)).toHaveBeenCalledWith("vue/package.json", {
        url: "/project",
      });
    });
  ...
  describe("fetchNpmPackage", () => {
    ...
    it("fetches package info from npm registry", async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith("https://unpkg.com/vue/package.json", undefined);
    });
...
---`);
  ...
  describe("resolvePackageDocs", () => {
    ...
    it("skips homepage if its a social media URL", async () => {
      ...
      vi.mocked(fetchGitHubRepoMeta).mockResolvedValue({
