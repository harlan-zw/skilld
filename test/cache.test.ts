...
describe('cache', () => {
  describe('getVersionKey', () => {
    it('extracts major.minor from semver', () => {
      ...
      expect(getVersionKey('0.0.1')).toBe('0.0')
    })

    it('handles prerelease versions', () => {
      expect(getVersionKey('1.2.3-beta.1')).toBe('1.2')
      expect(getVersionKey('2.0.0-rc.1')).toBe('2.0')
    })
  ...
  describe('getCacheKey', () => {
    it('combines name and version key', () => {
      expect(getCacheKey('vue', '3.4.21')).toBe('vue@3.4')
      expect(getCacheKey('@nuxt/kit', '3.10.0')).toBe('@nuxt/kit@3.10')
    })
  })
  ...
  describe('linkReferences', () => {
    ...
    it('creates symlink without unlinking if none exists', async () => {
      const { existsSync, unlinkSync, symlinkSync } = await import('node:fs')
      const { linkReferences } = await import('../src/cache')
      vi.mocked(existsSync).mockReturnValue(false)
