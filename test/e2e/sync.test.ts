...
import { ensureCacheDir, getCacheDir, getPackageDbPath, getShippedSkills } from "../../src/cache";
  ...
  for (const pkg of PACKAGES) {
    describe(pkg.name, () => {
      function get(): PipelineResult {
        ...
        if (!r) throw new Error(`No result for ${pkg.name}`);
        return r;
      }
      ...
      if (!pkg.expectShipped && !process.env.CI) {
        it("search.db valid if present", () => {
          const r = get();
          if (r.docsType === "llms.txt") return;
          const dbPath = getPackageDbPath(pkg.name, r.version);
          if (!hasValidSearchDb(dbPath)) return; // No valid index — skip (may be cached from prior run)
