...
describe("skilld add (no agent) + assemble", () => {
  ...
  describe("skilld assemble", () => {
    ...
    beforeAll(() => {
      ...
      writeFileSync(
        join(ASSEMBLE_DIR, "_BEST_PRACTICES.md"),
        `## Best Practices
...
`,
      ...
      writeFileSync(
        join(ASSEMBLE_DIR, "_API_CHANGES.md"),
        `## API Changes (v1.x)
...
`,
  ...
  describe("skilld assemble (invalid content)", () => {
    ...
    beforeAll(() => {
      ...
      writeFileSync(
        join(BAD_DIR, "_BEST_PRACTICES.md"),
        "This is just plain text with no headings or structure at all.",
      );
    });
