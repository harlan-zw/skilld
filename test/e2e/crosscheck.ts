...
async function main() {
  ...
  if (flag === "--json") console.log(formatJson(rows));
  else if (flag === "--md") console.log(formatMarkdown(rows));
...
const isMain =
  process.argv[1]?.endsWith("crosscheck.ts") || process.argv[1]?.endsWith("crosscheck");
if (isMain) main();
