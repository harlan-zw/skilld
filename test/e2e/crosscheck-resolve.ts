...
function classifyTier(row: ResolveRow): ResolveTier {
  ...
  if (row.gitDocsUrl && row.gitDocsFiles > 0) return "git-docs";
  if (row.llmsUrl) return "llms-txt";
  if (row.readmeUrl) return "readme";
  return "no-docs";
}
...
function detectIssues(row: ResolveRow, attempts: ResolveAttempt[]): string[] {
  ...
  if (row.docsUrl && isUselessDocsUrl(row.docsUrl)) {
    ...
    try {
      ...
      else issues.push("social-media-homepage");
    } catch {
      issues.push("social-media-homepage");
    }
  }

  if (!row.repoUrl) issues.push("no-repo-url");
  ...
  if (row.repoUrl && !row.gitDocsUrl) issues.push("no-git-docs");

  if (row.docsUrl && !row.llmsUrl) issues.push("no-llms-txt");

  if (!row.readmeUrl) issues.push("no-readme");
...
function formatSummary(rows: ResolveRow[]): string {
  const tiers: Record<ResolveTier, number> = {
    "git-docs": 0,
    "llms-txt": 0,
    readme: 0,
    "no-docs": 0,
...
function formatActionable(rows: ResolveRow[]): string {
  ...
  const candidates = rows.filter(
    (r) => r.status === "ok" && r.docsUrl && !r.gitDocsUrl && !r.llmsUrl && r.tier === "readme",
  );
...
async function main() {
  ...
  if (flags.includes("--json")) console.log(formatJson(rows));
  else if (flags.includes("--md")) console.log(formatMarkdown(rows));
