...
export async function runPipeline(name: string): Promise<PipelineResult> {
  ...
  const hasCachedDocs = cachedDocFiles.some(
    (f) =>
      f.startsWith("docs/") ||
      f.startsWith("src/") ||
      f === "llms.txt" ||
  ...
  if (hasCachedDocs) {
          ...
          for (let i = 0; i < gitDocs.files.length; i += BATCH) {
            ...
            const results = await Promise.all(
              batch.map(async (file) => {
                ...
                const res = await fetch(url, { headers: { "User-Agent": "skilld/1.0" } }).catch(
                  () => null,
                );
            ...
            for (const r of results) {
              if (r) {
                ...
                const stripped = gitDocs.docsPrefix
                ...
                docsToIndex.push({
                  id: cachePath,
                  content: r.content,
                  metadata: { package: name, source: cachePath },
                });
              }
            }
          }
        ...
        if (llmsContent.links.length > 0) {
          ...
          for (const doc of docs) {
            ...
            docsToIndex.push({
              id: doc.url,
              content: doc.content,
              metadata: { package: name, source: `docs/${localPath}` },
            });
          }
    ...
    if (resolved.readmeUrl && cachedDocs.length === 0) {
      ...
      if (content) {
        ...
        docsToIndex.push({
          id: "README.md",
          content,
          metadata: { package: name, source: "docs/README.md" },
        });
      }
    }
    ...
    if (docsToIndex.length > 0 && !hasValidSearchDb(dbPath)) {
      await indexLimit(async () => {
        if (hasValidSearchDb(dbPath)) return;
  ...
  const fmLines = ["---", `name: ${skillDirName}`, `description: ${description}`];
  if (version) {
